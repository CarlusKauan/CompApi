...
namespace CompApi.Controllers
{
    ...
    public class CompromissosController : ControllerBase
    {
        ...
        //Métodos de Banco usando Dapper abaixo:
        ...
        public async Task<ActionResult> GetAllAsync()
        {
            using (IDbConnection conexao = ConnectionFactory.GetStringConexao(_config))
            {
                ...
                List<Compromisso> lista = await NewMethod(conexao, sql);
                return Ok(lista);
            }

            static async Task<List<Compromisso>> NewMethod(IDbConnection conexao, StringBuilder sql)
            {
                return (await conexao.QueryAsync<Compromisso>(sql.ToString())).ToList();
            }
        }
        [HttpGet("{id}")]
